	...
	$row	= mysql_fetch_array($result);
		...
		if(!$row['id']) {
			print '<div class="alert alert-fixed alert-warning alert-dismissible" role="alert">
								  ...
								  <p class="alert__text"> Выберите тарифный план.</p>
								</div>';
			...
			print '<div class="alert alert-fixed alert-warning alert-dismissible" role="alert">
								  ...
								  <p class="alert__text"> Сумма не соответствует выбраному тарифному плану.</p>
								</div>';
			...
			print '<div class="alert alert-fixed alert-warning alert-dismissible" role="alert">
								  ...
								  <p class="alert__text"> У вас недостаточно средств на счету Perfectmoney, рекомендуем <a href="/enter/">пополнить</a> его.</p>
								</div>';
			...
			print '<div class="alert alert-fixed alert-warning alert-dismissible" role="alert">
								  ...
								  <p class="alert__text"> У вас недостаточно средств на счету Payeer, рекомендуем <a href="/enter/">пополнить</a> его.</p>
								</div>';
			...
			print '<div class="alert alert-fixed alert-warning alert-dismissible" role="alert">
								  ...
								  <p class="alert__text"> У вас недостаточно средств на счету Bitcoin, рекомендуем <a href="/enter/">пополнить</a> его.</p>
								</div>';
			...
			print '<div class="alert alert-fixed alert-warning alert-dismissible" role="alert">
								  ...
								  <p class="alert__text"> Процент реинвестиций должет быть от 0 до 100</p>
								</div>';
		} else {
			...
			print '<div class="alert alert-fixed alert-success alert-dismissible" role="alert">
								  ...
								  <p class="alert__text"> Депозит открыт! <a href="/deposits/">К списку депозитов »</a></p>
								</div>';
		}
		...
		print '<div class="alert alert-fixed alert-warning alert-dismissible" role="alert">
								  ...
								  <p class="alert__text"> Выберите тарифный план, платежную систему и введите сумму депозита</p>
								</div>';
	}
...
print "
	<div class=\"planik\"><span class=\"cr-showcase\" style=\"position: relative;\"><input style=\"float: left;\" class=\"custom-radio\" type=\"radio\" name=\"plan\" id=\"plan".$row['id']."\" value=\"".$row['id']."\" checked /> <label for=\"plan".$row['id']."\"><div style=\"width:100%;\"><div style=\"text-align: center;\"><b style=\"position: relative; z-index: 1; top: 50px; font-size: 30px;\">".$row['name']."</b></div><hr class=\"hrs\"><span class=\"circle_figure\"></span>";
		print "<div style=\"width: 280px; position: relative; z-index: 1; left: 11px; top: 68px; font-size: 19px; text-align: center;\">Данный пакет включает в себя сумму инвестиций от $".$row['minsum']." до $".$row['maxsum']." под ".$row['percent']."% в ";
...
<td align="right" style="vertical-align: middle;">Сумма: </td>
	<td colspan="1" width="300px"><input style="width: 100%;" type="text" id="sum" class="summka" name="sum" value="<?php print $pmbalance; ?>" onblur="if (value == '') {value='Сумма'}" onfocus="if (value == 'Сумма') {value =''}" autocomplete="off" placeholder="Сумма"/></td>
</tr>
...
<tr>
	<td></td>
	<td><button type="submit" style="width:100%; margin: 2px 0 0 0;" class="btn green fixed">Открыть депозит</button></td>
</tr>
</table>
