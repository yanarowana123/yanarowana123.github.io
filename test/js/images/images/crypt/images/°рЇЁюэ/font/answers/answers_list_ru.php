	...
	if ($login) {
		if ($_POST['radio'] < 1 OR $_POST['radio'] > 2) {
			print '<div class="alert alert-fixed alert-danger alert-dismissible" role="alert">
								  ...
								  <p class="alert__text"> Не балуйтесь!</p>
								</div>';
		} else {
			...
			if (!$text || $text == " ") {
				...
				if (mysql_query($sql)) {
					print '	<div class="alert alert-fixed alert-success alert-dismissible" role="alert">
								  ...
								  <p class="alert__text">Сообщение добавлено!</p>
								</div>';
				} else {
					print '	<div class="alert alert-fixed alert-danger alert-dismissible" role="alert">
								  <button type="button" class="close" data-dismiss="alert"><span aria-hidden="true"><i class="pe-7s-close"></i></span><span class="sr-only">Close</span></button>
...
function topics_list($page, $num, $status)
{
	...
	print'<header class="widget__header one-btn">
								<div class="widget__title" style=" width: 100%;">
									<i class="pe-7f-ribbon"></i><h3>Отзывы</h3>
								</div>
							</header>';
...
<table width="100%" border="0" align="center">
	...
	<tr>
		<td colspan="3"><textarea placeholder="Текст сообщения..." style="min-width: 278px; width: 100%; background: rgba(0,0,0,0.25);" name="text" rows="7" cols="48"><?php print htmlspecialchars(substr($_POST['text'], 0, 10000), ENT_QUOTES, ''); ?></textarea></td>
	</tr>
	<tr>
		<td style="width: 50%;">
		<label for="radios1">Положительный</label> <input class="check" type="radio" id="radios1" title="Положительный отзыв" name="radio" value="1" checked />
		 <label for="radios2">Отрицательный</label> <input class="check" type="radio" id="radios2" name="radio" title="Отрицательный отзыв" value="2" />
		</td>
		<td>
		</td>
