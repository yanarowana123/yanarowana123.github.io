...
*}
	...
	<br>
								{*** BUTTONS ***}
	{if $btn_text !== ' '}
<h4><input name="{$edit_form_name}_btn" value="{if $btn_text}{$btn_text}{elseif !$edit_is_new}Сохранить{else}Создать{/if}" type="submit" class="button-blue"></h4>
	{/if}
	{if is_array($btns)}
	...
	{*if !$hide_cancel and !$_IN}
		&nbsp;<a href="javascript:history.go(-1)" class="button-gray">Отменить</a>
	{/if*}
</form>
