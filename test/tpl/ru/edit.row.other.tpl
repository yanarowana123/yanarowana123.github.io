...
{else}							{*** PREPARE (FOR ALL) ***}
	...
	{if $v[0] == 'CC' or $v[0] == 'RR' or $v[0] == 'TTTT' or $v[0] == 'Y' or $v[0] == 'W' or $v[0] == 'XX' or $v[0] == 'L'}



			{if $v[0] == 'CC'}
				<td colspan='2' class='panel-checkbox act' >
					<div>
						<label class="chbx act">
							<input{$n}value="1"  checked="checked" type="checkbox" class="checkbox{$class}{$error_class}">
						</label>
						<span>{$l}</span>
					</div>
				</td>
			{elseif $v[0] == 'TTTT'}
				<td colspan='2'  class='panel-checkbox' >
					{if $v[1]}{$l}{/if}
					<input{$n}{$hint}value="{$cv}" size="{if $v.size > 0}{$v.size}{else}20{/if}"type="text" class="string_small{$class}{$error_class}">
				</td>
			{elseif $v[0] == 'RR'}
			<td colspan='2'  class='spec-td' align="center">
				{$error_text}
				<input{$n}value="{$v[3]}"{if $cv == $v[3]} checked="checked"{/if} type="radio" class="radio{$class}{$error_class}"> {$l}
			</td>
			{else}
			<td  align="right" width="{if $l_width}{$l_width}{else}0%{/if}">
	...
	{else}
		...
		<td align="left"{if $r_width} width="{$r_width}"{/if}>
			...
			{elseif $v[0] == 'CCC'}
				<input{$n}value="{$v[3]}"{if $cv == $v[3]} checked="checked"{/if} type="checkbox" class="checkbox_small{$class}{$error_class}">
			{elseif $v[0] == 'C'}
			...
			{elseif $v[0] == 'S'}
			<label class="select-css" style='width:100%;'>
