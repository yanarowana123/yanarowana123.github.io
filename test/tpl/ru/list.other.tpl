...
{if count($values)}
	...
	{/if}
	<table class="{if $table_class}{$table_class}{else}{/if} table tbl-style move-on" >
	<tr class='tr-th'>
		{if is_array($btns)}
			<th width="20px" class="thead">
	...
	<tr class="{$row_class}{if $id == $smarty.get.id}marked{elseif ($smarty.foreach.list.iteration % 2)}odd{else}even{/if} {if $l._Marked}new{/if}"  align="left">
		{if is_array($btns)}
			<td align="center">
